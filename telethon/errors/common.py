...
class BrokenAuthKeyError(Exception):
    ...
    def __init__(self):
        super().__init__(
            'The authorization key Sudah Berakhir, and Harus direset ulang.'
        )
...
class SecurityError(Exception):
    ...
    def __init__(self, *args):
        if not args:
            args = ['Hedwar mengatakan Chek Security Gagal.']
        super().__init__(*args)
...
class CdnFileTamperedError(SecurityError):
    ...
    def __init__(self):
        super().__init__(
            'File CDN has been altered and Hedwar membatalkan Download Data.'
        )
